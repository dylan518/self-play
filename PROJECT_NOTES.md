...
- **1-GPU colocated Stage-A does NOT fit** (4 attempts: service floor ~14GB [0.18; 0.12/0.15 KV-starve] + trainer fwd ~54GB > 80GB) — colocation needs ≥2 GPUs for the 4B. Fallback ran instead:
- **LIVE service test PASSED (97901, ~38min/2×96 Qs, 1 GPU):** real n=9 solve + K=10 verify fan-out through `compute_score` — rewards span −0.17…+0.95 (top = genuine band∧verified), live marginals mean_v≈0.37/mean_b≈0.03 (b matches offline replay), duals accumulate correctly across calls (λ_v 0.011→0.030, λ_b 0.029→0.057). Reward stack fully validated live; only verl-trainer integration remains (4-GPU 1-step smoke 97896 QUEUED behind f.qiao's 3-GPU job on 2305).
- **FOOTGUN found: `start_vllm_server.py` defaults `VERIFY_SUBSAMPLE=0`** — a standalone service launch silently skips ALL verification (votes/score_cv absent → every v=0, gate=0). iteration_rzero.sh masks this by exporting 1.0. Always export VERIFY_SUBSAMPLE=1.0 when launching the service outside the pipeline. (First svc-live run burned 30min on this: max reward exactly 0.000 across 192 questions = the tell.)
