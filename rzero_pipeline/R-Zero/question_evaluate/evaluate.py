...
if __name__ == "__main__" and _mp.parent_process() is None:
    ...
    model = vllm.LLM(
        ...
        dtype="bfloat16",
        max_model_len=int(os.getenv("EVAL_MAX_MODEL_LEN", "8192")),
    )
    ...
    for response, golden_answer, question in zip(responses, answers, questions):
        try:
            try:
                ...
                _dd = '/data/selfplay/solver_eval_dumps'; _os.makedirs(_dd, exist_ok=True)
                with open(_dd + '/' + args.save_name + '_' + str(args.suffix) + '.jsonl', 'a') as _df:
                    print(_json.dumps({'question': question, 'golden': golden_answer, 'solutions': [o.text for o in response.outputs]}), file=_df)
            except Exception:
                pass
            ...
            results = [r for r in results if r is not None and str(r).strip().lower() not in ("", "none")]
            if len(results) < 2:
                continue  # too few parseable answers to score reliably
