...
def main(args):
    ...
    model = vllm.LLM(
        ...
        dtype="bfloat16",
        max_model_len=int(os.getenv("GEN_MAX_MODEL_LEN", "8192")),
    )
    ...
    chat = [
        {
            ...
            "content": (
                ...
                + ("HARD REQUIREMENT: the problem must have a single answer that is a non-negative INTEGER "
                   "that a short Python program could compute by direct computation, enumeration, brute force, or simulation, and that prints as a single integer. "
                   "Good examples: counting objects with constraints, number-theoretic counts, exact probabilities as reduced fractions, "
                   "recursively defined quantities, optimization over finite sets. "
                   "Avoid: proof questions, construction questions, and any answer that is a decimal, fraction, list, set, irrational, or symbolic expression - the answer MUST be a single integer.\n"
                   if os.getenv("QUESTIONER_PROMPT", "").endswith("questioner_verifiable.jinja") else "")
                + "Aim for a difficulty such that fewer than 30 % of advanced high-school students could solve it. "
                "Avoid re-using textbook clichés or famous contest problems, and vary the mathematical structure rather than tweaking parameters of one template.\n"
        ...
        },
        {
            ...
            "content": (
                ...
                "<question>\nHow many ordered pairs of positive integers (a, b) with a, b <= 100 satisfy a*b = a + b + 50?\n</question>\n"
                "\\boxed{4}\n\n"
                "Now generate one new, DIFFERENT, challenging reasoning question. "
                "It must be well-posed and self-contained with exactly one correct non-negative integer answer. "
                "Format the output exactly as instructed."
            )
        }
    ]
